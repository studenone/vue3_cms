...
const analysisModule: Module<IAnalysis, IRootState> = {
  ...
  actions: {
    ...
    async getAnalysisDataAction({ commit }) {
      ...
      commit('changeCategoryGoodsCount', ({ data } = goodsCountRes))
      commit('changeCategoryGoodsSale', ({ data } = goodsSaleRes))
      commit('changeCategoryGoodsFavorite', ({ data } = goodsFavRes))
      commit('changeGoodsAddressSale', ({ data } = goodsAddressRes))
    },
  },
}
