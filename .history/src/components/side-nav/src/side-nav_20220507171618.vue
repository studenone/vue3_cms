...
import { pathMatchRoute, firstRoute } from '@/utils/map-menus'
...
export default defineComponent({
  ...
  setup() {
    ...
    const currentMenu = pathMatchRoute(menus.value, currentRoute.path) //当前路径匹配到的菜单
    const defaultIndex = computed(() => {
      return currentMenu ? currentMenu.id + '' : '2'
    })
    // 点击菜单跳转路由
